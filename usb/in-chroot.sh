...
while [[ -z "$hostname" ]]; do
    ...
    # 去掉前后/中间所有空白，避免误输入空格写入 /etc/hostname 导致 systemd-hostnamed 异常
    hostname="${hostname//[[:space:]]/}"
    if [[ -z "$hostname" ]]; then
        warn "Hostname cannot be empty"
        continue
    fi
    # RFC1123: 字母/数字开头结尾，中间可含 `-`；不允许首尾横线。
    # systemd-hostnamed 对违规 hostname 会拒绝设置，提前拦下避免重启后报错
    if [[ ! "$hostname" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?$ ]]; then
        warn "Hostname must start/end with alphanumeric, dashes allowed in between"
        hostname=""
    fi
done
