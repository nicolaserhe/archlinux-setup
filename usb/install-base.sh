...
# 幂等守卫：与 fstab/archlinuxcn 同样的模式，避免中途失败重跑时把 CN mirror
# 翻倍写入
if grep -q 'mirrors.tuna.tsinghua.edu.cn' /etc/pacman.d/mirrorlist; then
    warn "CN mirrors already in mirrorlist, skipping prepend"
else
