...
# BUG FIX: 加 -e，非 run_step 管控的命令失败时立即退出，而非静默继续
set -euo pipefail
...
# TARGET_USER 在 _step_user 中赋值；trap 里用 ${TARGET_USER:-} 守卫
TARGET_USER=""

# -- 全局清理（trap 保证无论如何都会执行）-------------------------------------
_cleanup() {
    # BUG FIX: 清理 trap 内部禁用 -e，防止中间命令失败打断后续清理
    set +e
    header "Cleanup"
...
_step_linger() {
    ...
    loginctl enable-linger "$TARGET_USER" \
        ...
        || { warn "loginctl enable-linger failed -- user services may need re-enabling at first login"; return 0; }
...
# 非致命：没有 linger 时退化到首次登录后手动处理
run_step "User linger" _step_linger || true

# -- Step 6: 用户阶段 ---------------------------------------------------------
# BUG FIX: REPO_DIR 用双引号包裹，防止路径中含空格时 runuser -c 参数断裂
run_step "User phase" \
